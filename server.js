...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f7f4;padding:40px 16px;">
  <tr><td align="center">
    ...
    if (method === 'POST' && url === '/api/export/pdf') {
      ...
      const firstISO  = entries.length > 0 ? entries[entries.length - 1].date : exportDate;
      const dateRange = entries.length > 0
        ? `${fmtDate(firstISO)} – ${fmtDate(exportDate)}`
        : `Exported ${fmtDate(exportDate)}`;

      const MOOD_LABELS = { awful: '😞 Awful', meh: '😐 Meh', okay: '🙂 Okay', good: '😊 Good', great: '🤩 Great' };

      const doc = new PDFDocument({ margin: 50, size: 'A4', bufferPages: true });

      // Collect into buffer — send headers only after full success
      const chunks = [];
      doc.on('data', chunk => chunks.push(chunk));
      doc.on('error', err => {
        console.error('[PDF export]', err);
        if (!res.headersSent) sendJSON(res, 500, { error: 'Failed to generate PDF. Please try again.' });
      });
      doc.on('end', () => {
      ...
      // ── COVER PAGE ─────────────────────────────────────────────
      doc.fontSize(30).font('Helvetica-Bold').fillColor('#1D9E75')
         .text('ReflectAI', { align: 'center' });
      doc.moveDown(0.4);
      doc.fontSize(13).font('Helvetica').fillColor('#444')
         .text('Personal Journal & Reflection Export', { align: 'center' });
      doc.moveDown(0.9);
      doc.moveTo(100, doc.y).lineTo(495, doc.y).strokeColor('#1D9E75').lineWidth(1).stroke();
      doc.moveDown(0.9);
      ...
      doc.fontSize(10).fillColor('#888')
         .text(`${entries.length} journal entr${entries.length === 1 ? 'y' : 'ies'}`, { align: 'center' });
      doc.moveDown(2.5);
      doc.fontSize(8).fillColor('#bbb')
         .text('Generated by ReflectAI — PremierLEADZ Consulting Ltd', { align: 'center' });
      ...
      if (entries.length === 0) {
        ...
        for (let i = 0; i < entries.length; i++) {
          ...
          if (i > 0 && doc.y > doc.page.height - 180) doc.addPage();
          ...
          // Mood line — prefer entry.mood, fall back to standalone mood log
          const moodKey = entry.mood || moodByDate[entry.date] || null;
          if (moodKey) {
            const moodLabel = MOOD_LABELS[moodKey] || moodKey.charAt(0).toUpperCase() + moodKey.slice(1);
            doc.fontSize(10).font('Helvetica').fillColor('#777').text(`Mood: ${moodLabel}`);
          }
          doc.moveDown(0.4);

          // Entry text
          doc.fontSize(10.5).font('Helvetica').fillColor('#333')
             .text(entry.text || '(No content)', { lineGap: 3 });

          // AI Reflection block with left border
          if (entry.coachSummary) {
            doc.moveDown(0.6);
            const rx     = 65;
            const rw     = doc.page.width - doc.page.margins.right - rx;
            const startY = doc.y;
            doc.fontSize(9).font('Helvetica-Bold').fillColor('#1D9E75')
               .text('Your AI Reflection 🌿', rx, doc.y, { width: rw });
            doc.moveDown(0.2);
            doc.fontSize(9).font('Helvetica-Oblique').fillColor('#555')
               .text(entry.coachSummary, rx, doc.y, { lineGap: 2, width: rw });
            // Left accent border
            doc.moveTo(57, startY - 2).lineTo(57, doc.y + 4)
               .strokeColor('#1D9E75').lineWidth(2).stroke();
          }
          ...
          if (i < entries.length - 1) {
            ...
            if (doc.y > doc.page.height - 160) {
              ...
              doc.moveTo(50, doc.y).lineTo(545, doc.y).strokeColor('#e8e8e8').lineWidth(0.5).stroke();
              doc.moveDown(1);
            }
          }
        }
      ...
      for (let p = 0; p < totalPages; p++) {
        ...
        doc.fontSize(8).font('Helvetica').fillColor('#bbb')
           .text('ReflectAI — PremierLEADZ Consulting Ltd', 50, footerY, { width: 300 });
        doc.fontSize(8).font('Helvetica').fillColor('#bbb')
